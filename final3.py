...
# Base features for PCA
base_features = ["Time", "Num_Cotton_Wicks", "Radiation", "Ambient_Temp", "Humidity", "Avg_Temp_With_Cooling"]
X_eff = data[base_features]

# Standardize input features
scaler_eff = StandardScaler()
X_scaled_eff = scaler_eff.fit_transform(X_eff)

# PCA (2 components for visualization)
pca_eff = PCA(n_components=2)
pca_eff_result = pca_eff.fit_transform(X_scaled_eff)
components_eff = pca_eff.components_

# Plot the PCA correlation circle
...
for i, feature in enumerate(base_features):
    x = components_eff[0, i]
    y = components_eff[1, i]
    plt.arrow(0, 0, x, y, color='b', alpha=0.6, head_width=0.03, length_includes_head=True)
    plt.text(x * 1.15, y * 1.15, feature, fontsize=10, ha='center', va='center',
             bbox=dict(facecolor='white', edgecolor='none', alpha=0.7))

plt.xlabel(f"PC1 ({pca_eff.explained_variance_ratio_[0]*100:.1f}%)")
plt.ylabel(f"PC2 ({pca_eff.explained_variance_ratio_[1]*100:.1f}%)")
plt.title("PCA Correlation Circle (Efficiency Model - Base Features)")
plt.axis('equal')
plt.grid(True)
plt.show()
...
data_grouped = data.groupby('Num_Cotton_Wicks')['Average_Power_with_Cooling'].mean().sort_index()
...
print(f"\nPredicted Average_Power_with_Cooling: {predicted_power[0]} W")

# Predict the power with cooling
predicted_power = lasso_model.predict(new_data_scaled)

plt.figure(figsize=(8, 6))
plt.scatter(data["Num_Cotton_Wicks"], data["Average_Power_with_Cooling"], color='b', alpha=0.6)

m, c = np.polyfit(data["Num_Cotton_Wicks"], data["Average_Power_with_Cooling"], 1)
plt.plot(data["Num_Cotton_Wicks"], m * data["Num_Cotton_Wicks"] + c, color='r', label=f'Fit Line: y = {m:.2f}x + {c:.2f}')

plt.title("Relationship Between Number of Cotton Wicks and Average Power With Cooling")
plt.xlabel("Number of Cotton Wicks")
plt.ylabel("Average Power With Cooling")
plt.legend()
plt.grid(True)
...
def plot_relationship(x_param, y_param):
    plt.figure(figsize=(8, 6))
    plt.scatter(data['Avg_Temp_With_Cooling'], data['Average_Power_with_Cooling'], alpha=0.7, color='b')
...
# Example: Relationship between Radiation and Average_Power_With_Cooling
plot_relationship("Avg_Temp_With_Cooling", "Average_Power_with_Cooling")
sns.pairplot(data)
plt.show()
