...
data.columns = [
    "Time","Num_Cotton_Wicks", "Radiation","Ambient_Temp", "Humidity",
     "Avg_Temp_With_Cooling", "Average_Efficiency_with_Cooling"
]

# Separate input (X) and output (Y)
X = data[["Time","Num_Cotton_Wicks", "Radiation", "Ambient_Temp", "Humidity","Avg_Temp_With_Cooling"]]
Y = data["Average_Efficiency_with_Cooling"]
...
# PCA (2 components for correlation circle)
...
# Line graph for Average Efficiency vs Number of Cotton Wicks
plt.figure(figsize=(8, 6))
data_grouped = data.groupby('Num_Cotton_Wicks')['Average_Efficiency_with_Cooling'].mean().sort_index()
plt.plot(data_grouped.index, data_grouped.values, marker='o', linestyle='-', color='b')
plt.title('Average Efficiency Output vs Number of Cotton Wicks')
...
# Predict the average efficiency with cooling
predicted_efficiency = lasso_model.predict(new_data_scaled)

plt.figure(figsize=(8, 6))
plt.scatter(data["Num_Cotton_Wicks"], data["Average_Efficiency_with_Cooling"], color='b', alpha=0.6)

m, c = np.polyfit(data["Num_Cotton_Wicks"], data["Average_Efficiency_with_Cooling"], 1)
plt.plot(data["Num_Cotton_Wicks"], m * data["Num_Cotton_Wicks"] + c, color='r', label=f'Fit Line: y = {m:.2f}x + {c:.2f}')

plt.title("Relationship Between Number of Cotton Wicks and Average Efficiency With Cooling")
plt.xlabel("Number of Cotton Wicks")
plt.ylabel("Average Efficiency With Cooling")
plt.legend()
plt.grid(True)
...
def plot_relationship(x_param, y_param):
    plt.figure(figsize=(8, 6))
    plt.scatter(data['Avg_Temp_With_Cooling'], data['Average_Efficiency_with_Cooling'], alpha=0.7, color='b')
...
# Example: Relationship between Radiation and Average_Efficiency_With_Cooling
plot_relationship("Avg_Temp_With_Cooling", "Average_Efficiency_with_Cooling")
sns.pairplot(data)
plt.show()
